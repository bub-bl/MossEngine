...
public abstract unsafe partial class MossWindow
{
	...
	protected virtual void OnBeforeRender( double deltaTime )
	{
		...
		_commandEncoder = WebGpuApi.Wgpu.DeviceCreateCommandEncoder( Device, null );

		WebGpuApi.Wgpu.SurfaceGetCurrentTexture( _surface, ref _surfaceTexture );
		...
		switch ( _surfaceTexture.Status )
		{
			...
			case SurfaceGetCurrentTextureStatus.Lost:
				WebGpuApi.Wgpu.TextureRelease( _surfaceTexture.Texture );
				ConfigureSurface();
				return;
			default:
		...
		_surfaceTextureView = WebGpuApi.Wgpu.TextureCreateView( _surfaceTexture.Texture, null );
	...
	{
		WebGpuApi.Wgpu.RenderPassEncoderEnd( RenderPassEncoder );
		WebGpuApi.Wgpu.RenderPassEncoderRelease( RenderPassEncoder );
		WebGpuApi.Wgpu.TextureViewRelease( _surfaceTextureView );

		var commandBuffer = WebGpuApi.Wgpu.CommandEncoderFinish( _commandEncoder, null );

		WebGpuApi.Wgpu.QueueSubmit( _queue, 1, &commandBuffer );

		WebGpuApi.Wgpu.CommandEncoderRelease( _commandEncoder );
		WebGpuApi.Wgpu.CommandBufferReference( commandBuffer );
