...
internal class BaseInputText : Panel
{
	...
	public override void Draw( SKCanvas canvas )
	{
		...
		var contentWidth = LayoutWidth;
		var contentHeight = LayoutHeight;
		...
		// Text / Placeholder
		if ( string.IsNullOrEmpty( Text ) && !string.IsNullOrEmpty( Placeholder ) && !_isFocused )
		{
	...
	protected override void OnKeyUp( KeyEventArgs args )
	{
		...
		if ( args.Key == _currentArrowKey &&
		     args.Key is Key.Left or Key.Right or Key.Home or Key.End )
		{
			_arrowKeyDown = false;
	...
	{
		var newPos = Math.Clamp( _cursorPosition + delta, 0, Text.Length );
	...
	private void InsertText( string textToInsert )
	{
		...
		if ( Text.Length + textToInsert.Length > MaxLength )
		{
			textToInsert = textToInsert[..Math.Max( 0, MaxLength - Text.Length )];
		}
