...
internal static class SplineUtils
{
		...
		float maxSquareError = 0.1f )
	{
		...
		outPoints.Capacity = System.Math.Max( outPoints.Capacity, numSegments * averagePointsPerSegmentEstimate );
		...
		int iterationNum )
	{
		...
		for ( int iteration = 0; iteration < iterationNum; ++iteration )
		{
			...
			currentCandidate = System.Math.Clamp( movedCandidate, 0.0f, 1.0f );
		}
	...
	public class SplineSampler
	{
			...
			float value )
		{
			...
			float clampedValue = System.Math.Clamp( value, inputMin, inputMax );
