...
public struct Angles : IEquatable<Angles>, IParsable<Angles>, IInterpolator<Angles>
{
	...
	{
		const float piOver180 = (float)(System.Math.PI / 180.0);

		float[] vAngles = { ang.yaw, ang.pitch };
		float[] vSines = new float[2];
		float[] vCosines = new float[2];

		vSines[0] = (float)System.Math.Sin( vAngles[0] * piOver180 );
		vSines[1] = (float)System.Math.Sin( vAngles[1] * piOver180 );

		vCosines[0] = (float)System.Math.Cos( vAngles[0] * piOver180 );
		vCosines[1] = (float)System.Math.Cos( vAngles[1] * piOver180 );
