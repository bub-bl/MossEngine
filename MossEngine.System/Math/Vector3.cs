...
using MossEngine.UI.Math.Interpolation;
...
public partial struct Vector3 : System.IEquatable<Vector3>, IParsable<Vector3>, IInterpolator<Vector3>
{
	...
	{
		var a = new Vector3( System.Math.Min( min.x, max.x ), System.Math.Min( min.y, max.y ), System.Math.Min( min.z, max.z ) );
		var b = new Vector3( System.Math.Max( min.x, max.x ), System.Math.Max( min.y, max.y ), System.Math.Max( min.z, max.z ) );
	...
	{
		if ( System.Math.Abs( x - v.x ) > delta ) return false;
		if ( System.Math.Abs( y - v.y ) > delta ) return false;
		if ( System.Math.Abs( z - v.z ) > delta ) return false;
	...
	public static Angles VectorAngle( in Vector3 vec )
	{
		...
		{
			yaw = (float)(System.Math.Atan2( vec.y, vec.x ) * 180.0f / System.Math.PI);
			if ( yaw < 0.0f )
			{
				yaw += 360.0f;
			}
			tmp = (float)System.Math.Sqrt( vec.x * vec.x + vec.y * vec.y );
			pitch = (float)(System.Math.Atan2( -vec.z, tmp ) * 180.0f / System.Math.PI);
			if ( pitch < 0.0f )
			{
				pitch += 360.0f;
			}
