...
public static partial class SandboxSystemExtensions
{
	...
	public static bool TryToType( this string str, Type t, out object Value )
	{
		...
		if ( t == typeof( Angles ) ) { Value = global::MossEngine.UI.Math.Angles.Parse( str ); return true; }
		if ( t == typeof( Color ) ) { Value = global::MossEngine.UI.Graphics.Color.Parse( str ); return true; }
		if ( t == typeof( Math.RangedFloat ) ) { Value = Math.RangedFloat.Parse( str ); return true; }
		if ( t.IsEnum ) { Value = Enum.Parse( t, str ); return true; }

		if ( t == typeof( Rotation ) )
		{
			Value = global::MossEngine.UI.Math.Rotation.Parse( str );

			// Special case when loading data from FGD angles
			if ( (Rotation)Value == global::MossEngine.UI.Math.Rotation.Identity )
			{
				var ang = global::MossEngine.UI.Math.Angles.Parse( str );
				Value = global::MossEngine.UI.Math.Rotation.From( ang );
			}
	...
	public static int Distance( this string source, string target )
	{
		...
		for ( var i = 1; i <= n; ++i )
		{
			...
			for ( var j = 1; j <= m; j++ )
			{
				...
				distance[currentRow, j] = System.Math.Min( System.Math.Min(
