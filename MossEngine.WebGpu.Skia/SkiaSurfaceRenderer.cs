...
namespace MossEngine.WebGpu.Skia;
...
public sealed class SkiaRenderPipeline : IDisposable
{
		...
		TextureFormat targetFormat, Vector2D<int> size, Action<SKCanvas, Vector2D<int>> draw )
	{
		...
		WebGpuApi.Wgpu.RenderPassEncoderSetPipeline( encoder, _pipeline );
		WebGpuApi.Wgpu.RenderPassEncoderSetBindGroup( encoder, 0, _activeBindGroup, 0, null );
		WebGpuApi.Wgpu.RenderPassEncoderDraw( encoder, 6, 1, 0, 0 );
	}

	private static bool IsQueueValid( WebGpuQueue queue ) => queue is not null;
	...
	private unsafe void UploadToTexture( WebGpuQueue queue, Texture* texture )
	{
		...
		WebGpuApi.Wgpu.QueueWriteTexture( queueHandle, &destination, pixelPtr, (nuint)_pixelBuffer.Length, &dataLayout,
			&writeSize );
	}
	...
	private unsafe void EnsureTexture( WebGpuDevice device, Vector2D<int> size )
	{
		...
		_texture = WebGpuApi.Wgpu.DeviceCreateTexture( device, desc );
		TextureView = WebGpuApi.Wgpu.TextureCreateView( _texture, null );
		TextureSize = size;
	}
	...
	private unsafe void ReleaseTexture()
	{
		...
		{
			WebGpuApi.Wgpu.TextureViewRelease( TextureView );
			TextureView = null;
		}

		if ( _texture is not null )
		{
			WebGpuApi.Wgpu.TextureRelease( _texture );
			_texture = null;
		}
	}
	...
	private unsafe void EnsurePipeline( WebGpuDevice device, TextureFormat targetFormat )
	{
		...
		_sampler = WebGpuApi.Wgpu.DeviceCreateSampler( device, samplerDesc );
		...
		_bindGroupLayout = WebGpuApi.Wgpu.DeviceCreateBindGroupLayout( device, layoutDesc );
		...
		_pipelineLayout = WebGpuApi.Wgpu.DeviceCreatePipelineLayout( device, pipelineLayoutDesc );
		...
		_pipeline = WebGpuApi.Wgpu.DeviceCreateRenderPipeline( device, pipelineDesc );
	...
	private unsafe void EnsureBindGroup( WebGpuDevice device, TextureView* textureView )
	{
		...
		{
			WebGpuApi.Wgpu.BindGroupRelease( _activeBindGroup );
		...
		_activeBindGroup = WebGpuApi.Wgpu.DeviceCreateBindGroup( device, bindGroupDesc );
	...
	private unsafe void DestroyPipelineResources()
	{
		...
		{
			WebGpuApi.Wgpu.RenderPipelineRelease( _pipeline );
			_pipeline = null;
		}

		if ( _sampler is not null )
		{
			WebGpuApi.Wgpu.SamplerRelease( _sampler );
			_sampler = null;
		}

		if ( _pipelineLayout is not null )
		{
			WebGpuApi.Wgpu.PipelineLayoutRelease( _pipelineLayout );
			_pipelineLayout = null;
		}

		if ( _bindGroupLayout is not null )
		{
			WebGpuApi.Wgpu.BindGroupLayoutRelease( _bindGroupLayout );
