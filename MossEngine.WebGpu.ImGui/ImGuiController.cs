...
public unsafe class ImGuiController : IDisposable
{
	...
	private BindGroup* BindImGuiTextureView( TextureView* view )
	{
		...
		var bindGroup = WebGpuApi.Wgpu.DeviceCreateBindGroup( _device, in imageDesc );
	...
	{
		ImGuiNET.ImGui.GetIO().Fonts.GetTexDataAsRGBA32( out byte* pixels, out var width, out var height, out var sizePerPixel );
		...
		_fontTexture = WebGpuApi.Wgpu.DeviceCreateTexture( _device, in textureDescriptor );
		...
		_fontView = WebGpuApi.Wgpu.TextureCreateView( _fontTexture, in textureViewDescriptor );
		...
		WebGpuApi.Wgpu.QueueWriteTexture( _queue, &imageCopyTexture, pixels, (nuint)(width * height * sizePerPixel),
		...
		_fontSampler = WebGpuApi.Wgpu.DeviceCreateSampler( _device, in samplerDescriptor );

		ImGuiNET.ImGui.GetIO().Fonts.SetTexID( (nint)_fontView );
	}
	...
	private void InitBindGroupLayouts()
	{
		...
		_commonBindGroupLayout = WebGpuApi.Wgpu.DeviceCreateBindGroupLayout( _device, in commonBgLayoutDesc );
		_imageBindGroupLayout = WebGpuApi.Wgpu.DeviceCreateBindGroupLayout( _device, in imageBgLayoutDesc );
	}
	...
	private void InitPipeline()
	{
		...
		var layout = WebGpuApi.Wgpu.DeviceCreatePipelineLayout( _device, in layoutDesc );
		...
		_renderPipeline = WebGpuApi.Wgpu.DeviceCreateRenderPipeline( _device, in renderPipelineDescriptor );

		SilkMarshal.Free( vertexEntry );
		SilkMarshal.Free( fragmentEntry );

		WebGpuApi.Wgpu.PipelineLayoutRelease( layout );
	}
	...
	private void InitUniformBuffers()
	{
		...
		_uniformsBuffer = WebGpuApi.Wgpu.DeviceCreateBuffer( _device, in bufferDescriptor );
	}
	...
	private void InitBindGroups()
	{
		...
		_commonBindGroup = WebGpuApi.Wgpu.DeviceCreateBindGroup( _device, in bgCommonDesc );
	...
	private void DrawImGui( RenderPassEncoder* encoder )
	{
		...
		if ( drawData.TotalVtxCount > 0 )
		{
			...
			WebGpuApi.Wgpu.QueueWriteBuffer( _queue, frameRenderBuffer.VertexBufferGpu, 0,
				frameRenderBuffer.VertexBufferMemory, (nuint)vertSize );
			WebGpuApi.Wgpu.QueueWriteBuffer( _queue, frameRenderBuffer.IndexBufferGpu, 0,
		...
		WebGpuApi.Wgpu.QueueWriteBuffer( _queue, _uniformsBuffer, 0, &uniforms, (nuint)sizeof( Uniforms ) );
		WebGpuApi.Wgpu.RenderPassEncoderSetPipeline( encoder, _renderPipeline );

		if ( drawData.TotalVtxCount > 0 )
		{
			WebGpuApi.Wgpu.RenderPassEncoderSetVertexBuffer( encoder, 0, frameRenderBuffer.VertexBufferGpu, 0,
				frameRenderBuffer.VertexBufferSize );
			WebGpuApi.Wgpu.RenderPassEncoderSetIndexBuffer( encoder, frameRenderBuffer.IndexBufferGpu, IndexFormat.Uint16,
				0,
				frameRenderBuffer.IndexBufferSize );
			uint dynamicOffsets = 0;
			WebGpuApi.Wgpu.RenderPassEncoderSetBindGroup( encoder, 0, _commonBindGroup, 0, in dynamicOffsets );
		}

		WebGpuApi.Wgpu.RenderPassEncoderSetViewport( encoder, 0, 0, drawData.FramebufferScale.X * drawData.DisplaySize.X,
		...
		for ( var n = 0; n < drawData.CmdListsCount; n++ )
		{
			...
			for ( var i = 0; i < cmdList.CmdBuffer.Size; i++ )
			{
				...
				else
				{
					...
					if ( texId != IntPtr.Zero )
					{
						if ( _viewsById.TryGetValue( texId, out var value ) )
						{
							...
							WebGpuApi.Wgpu.RenderPassEncoderSetBindGroup( encoder, 1, (BindGroup*)value, 0,
								in dynamicOffsets );
						}
					}
				}
				...
				WebGpuApi.Wgpu.RenderPassEncoderSetScissorRect( encoder, (uint)clipMin.X, (uint)clipMin.Y,
					(uint)(clipMax.X - clipMin.X), (uint)(clipMax.Y - clipMin.Y) );
				WebGpuApi.Wgpu.RenderPassEncoderDrawIndexed( encoder, cmd.ElemCount, 1, (uint)(idxOffset + cmd.IdxOffset),
	...
	private void CreateOrUpdateBuffers( ref FrameRenderBuffer frameRenderBuffer, ulong vertSize, ulong indexSize )
	{
		if ( frameRenderBuffer.VertexBufferGpu is null || frameRenderBuffer.VertexBufferSize < vertSize )
		{
			...
			{
				WebGpuApi.Wgpu.BufferDestroy( frameRenderBuffer.VertexBufferGpu );
				WebGpuApi.Wgpu.BufferRelease( frameRenderBuffer.VertexBufferGpu );
			}

			var desc = new BufferDescriptor { Size = vertSize, Usage = BufferUsage.Vertex | BufferUsage.CopyDst, };

			frameRenderBuffer.VertexBufferGpu = WebGpuApi.Wgpu.DeviceCreateBuffer( _device, in desc );
		...
		if ( frameRenderBuffer.IndexBufferGpu is null || frameRenderBuffer.IndexBufferSize < indexSize )
		{
			...
			{
				WebGpuApi.Wgpu.BufferDestroy( frameRenderBuffer.IndexBufferGpu );
				WebGpuApi.Wgpu.BufferRelease( frameRenderBuffer.IndexBufferGpu );
			}

			var desc = new BufferDescriptor { Size = indexSize, Usage = BufferUsage.Index | BufferUsage.CopyDst, };

			frameRenderBuffer.IndexBufferGpu = WebGpuApi.Wgpu.DeviceCreateBuffer( _device, in desc );
	...
	public void Dispose()
	{
		if ( _windowRenderBuffers.FrameRenderBuffers is not null )
		{
			...
			{
				WebGpuApi.Wgpu.BufferDestroy( renderBuffer.VertexBufferGpu );
				WebGpuApi.Wgpu.BufferRelease( renderBuffer.VertexBufferGpu );
				WebGpuApi.Wgpu.BufferDestroy( renderBuffer.IndexBufferGpu );
				WebGpuApi.Wgpu.BufferRelease( renderBuffer.IndexBufferGpu );
		...
		{
			WebGpuApi.Wgpu.BindGroupRelease( (BindGroup*)bg.Value );
		}

		WebGpuApi.Wgpu.BindGroupRelease( _commonBindGroup );

		WebGpuApi.Wgpu.BufferDestroy( _uniformsBuffer );
		WebGpuApi.Wgpu.BufferRelease( _uniformsBuffer );

		WebGpuApi.Wgpu.RenderPipelineRelease( _renderPipeline );

		WebGpuApi.Wgpu.BindGroupLayoutRelease( _commonBindGroupLayout );
		WebGpuApi.Wgpu.BindGroupLayoutRelease( _imageBindGroupLayout );

		WebGpuApi.Wgpu.SamplerRelease( _fontSampler );
		WebGpuApi.Wgpu.TextureViewRelease( _fontView );
		WebGpuApi.Wgpu.TextureDestroy( _fontTexture );
		WebGpuApi.Wgpu.TextureRelease( _fontTexture );
